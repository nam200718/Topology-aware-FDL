"""
Advanced High-Performance HEP Engine.
Synthesizes:
1. Multi-Head Parameter Isolation (fc2_root, fc2_parent, fc2_local)
2. Shannon Entropy Head-Epoch Scheduling (e_r, e_p, e_l) with late-epoch local specialization
3. Mini-batch Gradient-Adapted Alpha Simplex Optimization (Eq. 23)
4. Class-Frequency Balanced Active Class Logit Masking (CF-ACLM)
5. True IID Hard Collapse (R_skew >= 0.90 -> pure FedAvg)
6. Temperature-Calibrated Inference Blending (Tl=0.6, Tp=0.8, Tr=1.0)
7. Server-Side Nesterov Cluster Momentum (beta_c=0.85)
"""
...
def compute_head_epochs(r_skew: float, total_budget: int = 5):
    """
    Computes adaptive head-epoch allocation based on R_skew:
    - IID (R_skew >= 0.90): Pure Root (5, 0, 0)
    - Mild/Uniform (0.70 <= R_skew < 0.90): Prioritize Root & Parent (5, 3, 2)
    - Moderate (0.30 <= R_skew < 0.70): Balanced (4, 3, 3)
    - Severe/Extreme (R_skew < 0.30): Prioritize Local (2, 3, 5)
    """
    rem = max(0, total_budget - 3)
    if r_skew >= 0.90:
        return total_budget, 0, 0
    elif r_skew >= 0.70:
        e_root = 1 + round(rem * 0.6) + 2  # 5
        e_parent = 1 + round(rem * 0.3) + 1  # 3
        e_local = 1 + round(rem * 0.1)      # 2
        return e_root, e_parent, e_local
    elif r_skew < 0.30:
        e_local = 1 + round(rem * 0.6) + 2  # 5
        e_parent = 1 + round(rem * 0.3) + 1  # 3
        e_root = 1 + round(rem * 0.1)       # 2
        return e_root, e_parent, e_local
    else:
        e_root = 1 + round(rem * 0.34) + 1  # 4
        e_parent = 1 + round(rem * 0.33) + 1  # 3
        e_local = 1 + round(rem * 0.33) + 1  # 3
        return e_root, e_parent, e_local


def run_advanced_hep_benchmark(num_clients: int = 15, num_rounds: int = 20, batch_size: int = 64):
    ...
    for alpha_val, label in zip(alphas, alpha_labels):
        print(f"\n" + "="*70)
        print(f"Evaluating Advanced High-Performance HEP on CIFAR-10: {label}")
        print("="*70)
        ...
        for cid in range(num_clients):
            ...
            er, ep, el = compute_head_epochs(r_sk)
            client_head_epochs.append((er, ep, el))

            # Initial simplex prior: [local, parent, root]
            if r_sk >= 0.90:
                pi_prior = torch.tensor([-10.0, -10.0, 5.0], dtype=torch.float32, device=device)
            else:
                pi_root = min(1.0, max(0.0, r_sk ** 1.0))
                pi_local = (1.0 - pi_root) * (1.0 - r_sk)
                pi_parent = max(0.0, 1.0 - pi_root - pi_local)
                pi_prior = torch.log(torch.tensor([pi_local, pi_parent, pi_root], dtype=torch.float32, device=device) + 1e-4)
            client_alpha_states.append(pi_prior.clone())
        ...
        for r in range(num_rounds):
            ...
            for cid in range(num_clients):
                ...
                er, ep, el = client_head_epochs[cid]
                r_sk = client_r_skew[cid]
                mask = client_active_masks[cid]
                counts_t = client_counts_tensors[cid]
                alpha_t = client_alpha_states[cid]

                max_epochs = max(er, ep, el)
                local_m.train()
                ...
                for epoch in range(max_epochs):
                    for x, y in loader:
                        ...
                        loss_r = F.cross_entropy(zr, y) if epoch < er else 0.0
                        loss_p = F.cross_entropy(zp, y) if epoch < ep else 0.0
                        loss_l = loss_fn_cf_aclm(zl, y, active_mask=mask, class_counts=counts_t) if epoch < el else 0.0
                        ...
                        # Mini-batch Gradient Adaptation on Alpha Simplex (Eq. 23)
                        if r_sk < 0.90:
                            with torch.no_grad():
                                w_alpha = F.softmax(alpha_t, dim=0)
                                z_blend = w_alpha[0] * zl + w_alpha[1] * zp + w_alpha[2] * zr
                                p_blend = F.softmax(z_blend, dim=1)

                                target_onehot = torch.zeros_like(p_blend)
                                target_onehot.scatter_(1, y.unsqueeze(1), 1.0)
                                grad_logits = (p_blend - target_onehot) / y.size(0)

                                norm_factor = torch.clamp(torch.abs(zl).mean(), min=1e-4)
                                g_local = torch.sum(grad_logits * (zl / norm_factor))
                                g_parent = torch.sum(grad_logits * (zp / norm_factor))
                                g_root = torch.sum(grad_logits * (zr / norm_factor))

                                alpha_t[0] -= 0.05 * g_local * 0.1
                                alpha_t[1] -= 0.05 * g_parent * 0.1
                                alpha_t[2] -= 0.05 * g_root * 0.1

                client_alpha_states[cid].copy_(alpha_t)
        ...
        # Evaluation with Dynamic Confidence Weighting (Entropy + Loss + Alpha Prior)
        accs = []
        global_model.eval()
        for cid in range(num_clients):
            ...
            with torch.no_grad():
                for x, y in loader:
                    ...
                    if r_sk >= 0.90:
                        ...
                        # Dynamic Confidence Scoring: -H(p) + log(alpha_prior)
                        ...
                        alpha_vec = F.softmax(client_alpha_states[cid], dim=0)
                        score_l += np.log(max(1e-4, float(alpha_vec[0].item())))
                        score_p += np.log(max(1e-4, float(alpha_vec[1].item())))
                        score_r += np.log(max(1e-4, float(alpha_vec[2].item())))
    ...
    # -------------------------------------------------------------
    # CIFAR-100 Evaluation with Advanced High-Performance Engine
    # -------------------------------------------------------------
    print(f"\n" + "="*70)
    print("Evaluating Advanced High-Performance HEP on CIFAR-100 (Extreme Skew alpha=0.05)")
    print("="*70)
    ...
    client_counts100 = []
    client_head_epochs100 = []
    client_alpha_states100 = []
    ...
    for cid in range(num_clients):
        ...
        er, ep, el = compute_head_epochs(r_sk)
        client_head_epochs100.append((er, ep, el))

        pi_root = min(1.0, max(0.0, r_sk ** 1.0))
        pi_local = (1.0 - pi_root) * (1.0 - r_sk)
        pi_parent = max(0.0, 1.0 - pi_root - pi_local)
        pi_prior = torch.log(torch.tensor([pi_local, pi_parent, pi_root], dtype=torch.float32, device=device) + 1e-4)
        client_alpha_states100.append(pi_prior.clone())
    ...
    for r in range(num_rounds):
        ...
        for cid in range(num_clients):
            ...
            opt = torch.optim.SGD(local_m.parameters(), lr=0.05, momentum=0.9, weight_decay=1e-4, foreach=False)
            er, ep, el = client_head_epochs100[cid]
            mask = client_active_masks100[cid]
            counts_t = client_counts100[cid]
            alpha_t = client_alpha_states100[cid]
            ...
            client_alpha_states100[cid].copy_(alpha_t)
    ...
    for cid in range(num_clients):
        ...
        with torch.no_grad():
            for x, y in loader:
                ...
                alpha_vec = F.softmax(client_alpha_states100[cid], dim=0)
                score_l += np.log(max(1e-4, float(alpha_vec[0].item())))
                score_p += np.log(max(1e-4, float(alpha_vec[1].item())))
                score_r += np.log(max(1e-4, float(alpha_vec[2].item())))
    ...
    print(f"CIFAR-100 Advanced HEP Result: Mean Top-1 = {mean_acc100}%, Bottom-10% = {bot10_100}%")

    final_results = {
        "cifar10_advanced_hep": cifar10_results,
        "cifar100_advanced_hep": {
            "mean_acc": mean_acc100,
            "bottom_10": bot10_100,
        }
    }

    out_file = os.path.join(_project_root, "outputs", "advanced_hep_benchmark.json")
    os.makedirs(os.path.dirname(out_file), exist_ok=True)
    with open(out_file, "w") as f:
        json.dump(final_results, f, indent=2)
    print(f"\nAdvanced HEP Benchmark results saved to: {out_file}")
    return final_results


if __name__ == "__main__":
    run_advanced_hep_benchmark()

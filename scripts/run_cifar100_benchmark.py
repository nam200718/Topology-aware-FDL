"""
CIFAR-100 High-Class-Cardinality Benchmark (C = 100).

Evaluates FedAvg, FedRep, Ditto, and HEP on CIFAR-100 ResNet9
under Moderate (alpha = 0.5) and Extreme (alpha = 0.05) Non-IID Dirichlet skew.
...
def run_cifar100_experiment(num_clients: int = 15, num_rounds: int = 20, batch_size: int = 64):
    device = detect_device()
    print(f"Target Hardware Device: {device}")

    # Load CIFAR-100 (10,000 train samples, 3,000 test samples for fast edge simulation)
    train_raw, test_raw = get_cifar100(data_dir="./data", train_subset=10000, test_subset=3000, seed=42)
    
    # Preload to GPU memory
    print("Preloading CIFAR-100 datasets to GPU memory...")
    ...
    for sc_name, alpha in scenarios:
        ...
        print(f"  FedAvg Result: {scenario_res['FedAvg']['mean']:.2f}% (B10: {scenario_res['FedAvg']['bottom10']:.2f}%)")

        # -------------------------------------------------------------
        # 2. FedRep (Shared Base + Local Linear Head)
        # -------------------------------------------------------------
        print("\n[2/4] Training FedRep...")
        ...
        for r in range(num_rounds):
            ...
            for cid in range(num_clients):
                c_train = ClientDataset(train_fast, train_splits[cid])
                loader = get_fast_dataloader(c_train, batch_size=batch_size, shuffle=True)
                local_bb = ResNet9(in_channels=3, num_classes=100).to(device)
                ...
                # Phase 2: Train Backbone (3 epochs)
            ...
            # Server aggregation of backbone
            avg_state = {}
            for k in global_bb.state_dict().keys():
                avg_state[k] = torch.stack([client_bb_states[i][k].float() for i in range(num_clients)], dim=0).mean(dim=0)
        ...
        print(f"  FedRep Result: {scenario_res['FedRep']['mean']:.2f}% (B10: {scenario_res['FedRep']['bottom10']:.2f}%)")

        # -------------------------------------------------------------
        # 3. Ditto (Dual-Model Proximal PFL)
        # -------------------------------------------------------------
        print("\n[3/4] Training Ditto...")
        ...
        for r in range(num_rounds):
            ...
            for cid in range(num_clients):
                ...
                # Step 2: Personalized Model Training with Proximal Penalty
        ...
        print(f"  Ditto Result: {scenario_res['Ditto']['mean']:.2f}% (B10: {scenario_res['Ditto']['bottom10']:.2f}%)")

        # -------------------------------------------------------------
        # 4. HEP (Hierarchical Ensemble Personalization)
        # -------------------------------------------------------------
        print("\n[4/4] Training HEP (Ours)...")
        num_clusters = 3
        ...
        print(f"  HEP Result: {scenario_res['HEP (Ours)']['mean']:.2f}% (B10: {scenario_res['HEP (Ours)']['bottom10']:.2f}%)")

        all_results[sc_name] = scenario_res

    out_path = os.path.join(_project_root, "outputs", "cifar100_results.json")
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    with open(out_path, "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\nCIFAR-100 benchmark completed! Results saved to: {out_path}")
    return all_results
